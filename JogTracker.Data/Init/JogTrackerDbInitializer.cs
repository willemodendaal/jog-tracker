    ...
    public class JogTrackerDbInitializer : DropCreateDatabaseIfModelChanges<JogDbContext>
    {
        protected override void Seed(JogDbContext context)
        {
            context.JogEntries.AddRange(new[] {
                new JogEntry() { DistanceKM = 3F, DateTime = DateTime.Now.AddMonths(-2), Duration = new TimeSpan(0,30,2) },
                new JogEntry() { DistanceKM = 3.5F, DateTime = DateTime.Now.AddMonths(-1),  Duration = new TimeSpan(0,26,18)  },
            });
